...
ClientArgs* buildClient(char* name) {
    ...
    snprintf(clientArgs->qName, sizeof(clientArgs->qName), "/%s", clientArgs->name);
    clientArgs->qName[32] = '\0';

    clientArgs->mq = establishQueue(clientArgs->qName);
    int result = pthread_create(&clientArgs->threadId, NULL, client, (void*)clientArgs);
    if (result != 0) {
        errorPrint("Failed to create client thread");
        free(clientArgs);
        exit(EXIT_FAILURE);
    } else {
        debugPrint("Client thread created successfully with ID: %lu", (unsigned long)clientArgs->threadId);
    }
...
bool runTest_01() {
    ...
    ClientArgs* marina = buildClient("Marina");
    routine01(frederike);
    routine01(marina);
    routine01(leon);

    deconstuctClient(frederike);
    deconstuctClient(marina);
